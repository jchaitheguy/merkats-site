...
@keyframes mk-graze{0%,100%{height:calc(var(--s)*0.06)}6%,42%{height:calc(var(--s)*0.44)}48%{height:calc(var(--s)*0.06)}}
@keyframes mk-leaf{0%,4%,46%,100%{opacity:0}8%,40%{opacity:1}}
...
const meerkat = (size) => `<div class="mk" style="--s:${size}px;width:${size}px;height:${size * 1.12}px" aria-label="Merkat">
  ...
  <div class="anim" style="left:46%;top:72%;width:9%;height:calc(var(--s)*0.03);border-radius:45%;background:#6B3F26;animation:mk-chew 0.6s infinite"></div>
  <div class="anim" style="left:47.5%;bottom:30%;width:5%;height:calc(var(--s)*0.06);animation:mk-graze 4s ease-in-out infinite;overflow:visible">
